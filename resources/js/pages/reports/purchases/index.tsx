...
interface Props {
    details: PaginatedResponse<PurchaseDetail>;
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Reporte', href: '/reportpurchases' },
];
                ...
                <div className="rounded border p-6 shadow-xl">
                    ...
                    <div className="flex items-center justify-between mb-5 gap-2">
                        ...
                        <Button asChild variant="outline" size="sm" title="Filtrar">
                            <Link href={`/reportpurchases?from=${from}&to=${to}`} preserveScroll>
                                <Search className="h-4 w-4" />
                            </Link>
                        </Button>
                        <Button asChild variant="outline" size="sm" title="Limpiar">
                            <Link href="/reportpurchases" preserveScroll>
                                <X className="h-4 w-4" />
                            </Link>
                        </Button>
                        {/* Botón Excel */}
                        <Button variant="outline" size="sm" title="Exportar Excel" asChild>
                            <a
                                href={`/reportpurchases/export?from=${from}&to=${to}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                download="compras.xlsx"
                            >
                                📥
                            </a>
                        </Button>
                                    ...
                                    {details.data.length > 0 ? (
                                        details.data.map((detail, index) => (
                                            <TableRow key={detail.id}>
                                                ...
                                                <TableCell>{detail.purchase_date}</TableCell>
